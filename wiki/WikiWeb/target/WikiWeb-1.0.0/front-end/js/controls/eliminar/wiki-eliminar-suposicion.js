(function ($) {
    ...
    $.fn.wikiEliminarSuposicion = function (id) {
        ...
        if (id === "d-13") {
            ...
            $("#panel-foot-down").html("Suposicion");
            ...
            $("#panel-foot")
                    .append($("<div>").addClass("col-lg-12")
                            .append($("<div>").addClass("col-lg-10").attr({"id": "row-foot-13"})
                                    .append($("<table>").addClass("table table-hover")
                                            .append($("<thead>")
                                                    .append($("<tr>").addClass("active")
                                                            .append($("<th>").html("Id"))
                                                            .append($("<th>").html("Descripcion"))
                                                            .append($("<th>").html("Fuente"))
                                                            .append($("<th>").html(""))
                                                            )
                                                    )
                    ...
                    if (data !== undefined) {
                        ...
                        $("#tbody-13")
                                .append($("<tr>").attr({"id": data.id, "value": "13"})
                                        .append($("<td>").html("Suposicion " + data.id.split("_")[1]).attr({"width": "20%"}))
                                        .append($("<td>").html(data.description).attr({"width": "40%"}))
                                        .append($("<td>").html(data.source).attr({"width": "40%"}))
                                        .append($("<td>")
        ...
        function eventRemove() {
            ...
            ajaxDelete13(tableId);
            $(this).parent().parent().remove();

        }

        function ajaxDelete13(id) {
            $.ajax({
                url: "WikiWeb/assumption/delete",
                data: {
                    id: id
                },
                method: "POST"
            }).done(function () {
                swal({title: "Eliminacion Compeltada!!!", text: "Se elimino correctamente la suposicion", timer: 2000, showConfirmButton: false, type: "success"});
